...
import model.User
import network.{Board, ClientMem, GameClient, MainApp}
...
@sfxml
class MenuController(private var playerName: TextField,
  ...
  def playerJoin(action: ActionEvent): Unit = {
    if (playerName.text() != "") {
      ...
      ClientMem.ownRef.foreach(_ ! GameClient.StartJoin(playerName.text()))
    } else {
  ...
  def playerInvite(action: ActionEvent): Unit = {
    ...
    if (indexOpt < 0) { // No player selected
      ...
      if (!ClientMem.isHost) { // Only host can invite
      ...
      } else if (ClientMem.tableList.size >= 3) { // Room full
        new Alert(AlertType.Warning) {
          title = "Error"
          headerText = "Room full"
          contentText = "Wait for a player to leave before inviting again."
        }.showAndWait()
      } else if (userRefOpt == MainApp.ownRef) { // Check valid player
        new Alert(AlertType.Warning) {
          title = "Error"
        ...
        ClientMem.ownRef.foreach(userRefOpt ! GameClient.IsInvitable(_))
      }
    }
  }
  ...
  def playerInviteResult(from: User, result: Boolean): Unit = {
    if (result) {
      ClientMem.ownRef.foreach(_ ! GameClient.SendInvitation(from.ref))
    } else {
      new Alert(AlertType.Warning) {
        title = "Oops"
  ...
  def playerInvited(from: User): Unit = {
    ...
    result match {
      case Some(ButtonType.OK) => {
        ...
        ClientMem.ownRef.foreach(_ ! GameClient.AcceptInvitation(User(ClientMem.ownName, from.ref)))
      }
      case _ => ClientMem.ownRef.foreach(_ ! GameClient.RejectInvitation(User(ClientMem.ownName, from.ref)))
    }
  }
  ...
  def playerNew(action: ActionEvent): Unit = {
    ...
    leaveBtn.disable = false
    ClientMem.ownRef.foreach(_ ! GameClient.NewRoom)
  }
  ...
  def gameStart(action: ActionEvent): Unit = {
    ...
    leaveBtn.disable = true
    startBtn.disable = true
    for (user <- ClientMem.tableList) { //Inform all players to start game
      ClientMem.ownRef.foreach(_ ! GameClient.GameStart(user.ref))
    }
  }
